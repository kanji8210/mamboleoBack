<?php
/**
 * Country dataset — worldwide.
 *
 * Each entry: { name, slug, center:[lat,lng], bbox?:[minLat,minLng,maxLat,maxLng] }.
 * `bbox` is provided for Kenya + East-African neighbours so the auto-snap
 * keeps strict polygon checks for the local priority region. Other countries
 * carry only a centroid — `mamboleo_point_in_country()` accepts any point
 * when no bbox is present (worldwide pins are trusted as-is).
 *
 * Kenya remains the scraping priority — the Python pipeline boosts kenyan
 * sources/keywords; this file only drives the admin pickers and the
 * country-level fallback when an article mentions a foreign location.
 */
...
if ( ! function_exists( 'mamboleo_countries_data' ) ) {
    function mamboleo_countries_data(): array {
        ...
        // East African priority region — full bbox for in/out tests.
        $priority = [
        ...
        // Worldwide list — ISO 3166-1 (sans the 9 priority countries above).
        // Centroid only; bbox is intentionally omitted so any reasonable pin
        // inside the country is trusted by the auto-snap.
        $world = [
        ...
        $cache = array_merge( $priority, $rest );
        return $cache;
    }
}
...
if ( ! function_exists( 'mamboleo_point_in_country' ) ) {
    /**
     * True if (lat,lng) is inside the country bbox. When no bbox is recorded
     * (most countries outside the East African priority region) the point is
     * trusted as-is — worldwide pins are accepted without geometry checks.
     */
    function mamboleo_point_in_country( float $lat, float $lng, array $country ): bool {
        if ( empty( $country['bbox'] ) || ! is_array( $country['bbox'] ) ) {
            return true;
        }
